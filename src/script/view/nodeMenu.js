  ...
  initialize : function(data, tabs, otherCSSClass) {
    ...
    this.form.select('input.suggest-omim').each(function(item) {
      if (!item.hasClassName('initialized')) {
        // Create the Suggest.
        item._suggest = new PedigreePro.widgets.Suggest(item, {
          script: 'https://hpo.jax.org/api/hpo/search/?q=',
          queryProcessor: typeof(PedigreePro.widgets.SolrQueryProcessor) == 'undefined' ? null : new PedigreePro.widgets.SolrQueryProcessor({
        ...
        if (item.hasClassName('multi') && typeof(PedigreePro.widgets.SuggestPicker) != 'undefined') {
          item._suggestPicker = new PedigreePro.widgets.SuggestPicker(item, item._suggest, {
            'showKey' : false,
            'showTooltip' : false,
            'showDeleteTool' : true,
            'enableSort' : false,
