...
var ExportSelector = Class.create( {
  ...
  initialize: function() {
    ...
    var promptType = new Element('div', {'class': 'import-section'}).update(i18n.t('export.format') + ':');
    ...
    var label = new Element('label', {'class': 'export-config-header'}).insert(i18n.t('export.personIds'));
    configListElementPED.insert(label.wrap('td').wrap('tr'));
    configListElementPED.insert(_addConfigOption(true,  'ped-options', 'export-subconfig-label', i18n.t('export.externalId'), 'external'));
    configListElementPED.insert(_addConfigOption(false, 'ped-options', 'export-subconfig-label', i18n.t('export.name'), 'name'));
    configListElementPED.insert(_addConfigOption(false, 'ped-options', 'export-subconfig-label', i18n.t('export.generateNew'), 'newid'));

    var configListElementPrivacy = new Element('table', {'id': 'privacyOptions', 'style': 'display:none'});
    var privLabel = new Element('label', {'class': 'export-config-header'}).insert(i18n.t('export.privacy') + ':');
    configListElementPrivacy.insert(privLabel.wrap('td').wrap('tr'));
    configListElementPrivacy.insert(_addConfigOption(true,  'privacy-options', 'export-subconfig-label', i18n.t('labels.name'), 'all'));
    configListElementPrivacy.insert(_addConfigOption(false, 'privacy-options', 'export-subconfig-label', i18n.t('export.removeNames'), 'nopersonal'));
    configListElementPrivacy.insert(_addConfigOption(false, 'privacy-options', 'export-subconfig-label', i18n.t('export.removeComments'), 'minimal'));
    ...
    buttons.insert(new Element('input', {type: 'button', name : 'export', 'value': i18n.t('menu.export'), 'class' : 'button', 'id': 'export_button'}).wrap('span', {'class' : 'buttonwrapper'}));
    buttons.insert(new Element('input', {type: 'button', name : 'cancel', 'value': i18n.t('buttons.cancel'), 'class' : 'button secondary'}).wrap('span', {'class' : 'buttonwrapper'}));
    ...
    document.observe('language:changed', this.updateLanguage.bind(this));

    var closeShortcut = ['Esc'];
    this.dialog = new PedigreePro.widgets.ModalPopup(mainDiv, {close: {method : this.hide.bind(this), keys : closeShortcut}}, {extraClassName: 'pedigree-import-chooser', title: i18n.t('export.modalTitle'), displayCloseButton: true});
  },
  ...
  updateLanguage: function() {
    if (this.cancelButton) {
      this.cancelButton.value = i18n.t('buttons.cancel');
    }
    if (this.exportButton) {
      this.exportButton.value = i18n.t('menu.export');
    }
  }
});
