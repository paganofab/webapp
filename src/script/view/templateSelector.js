...
var TemplateSelector = Class.create( {
  ...
  initialize: function(isStartupTemplateSelector) {
    ...
    this.mainDiv.update(i18n.t('dialog.loadingTemplates'));
    var closeShortcut = isStartupTemplateSelector ? [] : ['Esc'];
    this.dialog = new PedigreePro.widgets.ModalPopup(this.mainDiv, {close: {method : this.hide.bind(this), keys : closeShortcut}}, {extraClassName: 'pedigree-template-chooser', title: i18n.t('templates.modalTitle'), displayCloseButton: !isStartupTemplateSelector, verticalPosition: 'top'});
    ...
    for (var i = 0; i < PedigreeTemplates.length; ++i) {
      ...
      pictureBox.update(i18n.t('messages.loading'));
