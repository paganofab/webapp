...
var PedigreeEditor = Class.create({
  initialize: function(options) {
    ...
    // Autosave functionality has been removed from PedigreePro
    // Users must manually save their pedigrees using the Save button

    if (backend.save === undefined || typeof backend.save !== 'function') {
      console.warn('No "save" function provided for backend - running in standalone mode');
    }
    if (backend.load === undefined || typeof backend.load !== 'function') {
      console.warn('No "load" function provided for backend - running in standalone mode');
    }
    ...
    this._workspace = new Workspace(options.container);
    ...
    unsupportedBrowserButton && unsupportedBrowserButton.on('click', function(event) {
      alert(i18n.t('dialog.unsupportedFeatures') + ' ' +
    ...
    // Autosave functionality removed - users must manually save pedigrees
